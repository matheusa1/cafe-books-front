...
const FooterModal: React.FC<IFooterModal> = ({ content, isOpen, setIsOpen, title }): ReactElement => {
  return (
    <Dialog.Root open={isOpen}>
      <Dialog.Portal>
        <Dialog.Overlay onClick={() => setIsOpen(false)} className="fixed inset-0 z-40 bg-black/50" />
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 flex w-[80vw] max-w-3xl -translate-x-1/2 -translate-y-1/2 flex-col gap-4 rounded-xl bg-white p-5">
          <header className="flex items-center justify-between">
            <div />
            <p>{title}</p>
            <X onClick={() => setIsOpen(false)} size={24} color="black" className="cursor-pointer" />
          </header>
